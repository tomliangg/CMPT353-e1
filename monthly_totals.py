...
def date_to_month(d):
    ...
    # input: datetime.datetime object (YYYY-MM-DD sec:milSec:micSec)
    # output: trim out the date part (YYYY-MM)
...
def pivot_months_pandas(data):
    ...
    sum_grouped_data = grouped_data.agg('sum').reset_index()  #sum the grouped data and generate a new DF
                                                            #the new DF got index reset to 0, 1, 2, ....
                                                            #name and month got pushed back to columns
    monthly = sum_grouped_data.pivot(index='name', columns='month', values='precipitation')

    count_grouped_data = grouped_data.agg('count').reset_index()
    counts = count_grouped_data.pivot(index='name', columns='month', values='precipitation')
...
def main1():
    ...
    sum_grouped_data = grouped_data.agg('sum').reset_index()  #sum the grouped data and generate a new DF
                                                            #the new DF got index reset to 0, 1, 2, ....
                                                            #name and month got pushed back to columns
    output_grouped_data = sum_grouped_data.pivot(index='name', columns='month', values='precipitation')
    #print (output_grouped_data)
    #output_grouped_data.to_csv('test.csv')

    count_grouped_data = grouped_data.agg('count').reset_index()
    output_grouped_data = count_grouped_data.pivot(index='name', columns='month', values='precipitation')
    output_grouped_data.to_csv('test1.csv')
